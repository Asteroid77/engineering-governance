...
- Every implementation must preserve the responsibility boundaries already established by the target repository. Public contracts should expose owned responsibilities rather than incidental implementation details, callers must not bypass responsibility owners through internal implementations, and boundary changes should be backed by structure or boundary tests where the repository has a testable boundary mechanism.
